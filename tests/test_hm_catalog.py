"""
Test script for H&M catalog extraction using Markdown Tower
Tests the MarkdownCatalogExtractor for catalog page extraction
Note: H&M uses hybrid pagination - markdown captures initially loaded products only
"""
...
# Test configuration
RETAILER = "hm"
TEST_URL = "https://www2.hm.com/en_us/women/products/dresses.html"
EXPECTED_MIN_PRODUCTS = 30  # H&M shows 36 products per page, allow some margin
EXPECTED_MAX_PRODUCTS = 40


async def test_hm_catalog_extraction():
    """Test H&M catalog extraction with Markdown Tower"""
    
    print("=" * 60)
    print("H&M CATALOG EXTRACTION TEST")
    print("=" * 60)
    ...
    try:
        ...
        # Determine PASS/FAIL (products >= 30 and success == True)
        # Note: H&M has hybrid pagination, shows 36 products per page initially
        ...
        print()
        print("=" * 60)
        print("Note: H&M uses hybrid pagination - markdown captures the")
        print("initially loaded products only (typically 36 per page).")
        print("This is expected behavior.")
        print("=" * 60)
        
        # Final status message
        if test_passed:
            print("✅ TEST PASSED: H&M catalog extraction working correctly!")
...
if __name__ == "__main__":
    print()
    result = asyncio.run(test_hm_catalog_extraction())
    print()
